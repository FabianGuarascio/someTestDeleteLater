...
export class MarkerService {
  ...
  createSvgMarker(
    ...
    if (tooltipText) {
      this.addTooltipEvents(svg, tooltipText, tooltipOptions);
    }
    return svg;
  }
  ...
  private addTooltipEvents(
    ...
    element.addEventListener('mouseenter', (event) => {
      this.showTooltip(event, text, options);
    });
  ...
  createSvgCustomPlusCricle( 
    ...
    const svgMarker = this.createSvgMarker(
      ...
      yLat,
      scale,
      tooltipText || `Count: ${badgeNumber}`, // Tooltip text
      tooltipOptions
    )
